...
<div class="card card-form" style="margin-top:16px">
    ...
    <form action="{{ route('departments.update', $department) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="dept_name" value="{{ old('dept_name', $department->dept_name) }}" required>
            @error('dept_name')<p class="error">{{ $message }}</p>@enderror
        </div>
        <div class="form-group">
            <label>Department Head</label>
            <input type="text" name="dept_head" value="{{ old('dept_head', $department->dept_head) }}">
