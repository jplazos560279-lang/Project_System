...
@section('content')
    ...
    <div class="card card-form" style="margin-top:0; padding-top:0; margin-left:auto; margin-right:auto; max-width: 900px;">
        ...
        function updateEmployeeBaseSalary() {
            ...
            const selectedOption = empSelect && empSelect.options ? empSelect.options[empSelect.selectedIndex] : null;
            const salary = selectedOption && selectedOption.dataset ? toNumber(selectedOption.dataset.salary) : 0;
    ...
    <form action="{{ route('payrolls.store') }}" method="POST" id="payrollForm">
        ...
        <fieldset class="form-section">
            ...
            <div class="form-group">
                ...
                <select name="emp_id" id="emp_id" required onchange="updateEmployeeBaseSalary()">
                    <option value="">Select an Employee</option>
                    @foreach($employees as $emp)
                        <option value="{{ $emp->emp_id }}" data-salary="{{ $emp->basic_salary ?? 0 }}" {{ old('emp_id') == $emp->emp_id ? 'selected' : '' }}>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="payroll_month" id="payroll_month" placeholder="YYYY-MM" value="{{ old('payroll_month') }}" readonly style="background: #f5f5f5;">
                </div>
            </div>
        </fieldset>
        ...
        <fieldset class="form-section">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" name="basic_salary" id="basic_salary" value="{{ old('basic_salary') }}" required onchange="calculateGross()">
                    @error('basic_salary')<p class="error">{{ $message }}</p>@enderror
                </div>
                <div class="form-group">
                    <label for="overtime_pay">Overtime Pay</label>
                    <input type="number" step="0.01" name="overtime_pay" id="overtime_pay" value="{{ old('overtime_pay', 0) }}" min="0" onchange="calculateGross()">
                    @error('overtime_pay')<p class="error">{{ $message }}</p>@enderror
                </div>
                <div class="form-group">
                    <label for="bonus">Bonus</label>
                    <input type="number" step="0.01" name="bonus" id="bonus" value="{{ old('bonus', 0) }}" min="0" onchange="calculateGross()">
        ...
        <fieldset class="form-section">
            <legend>Automatic Deductions (Optional)</legend>
            <p style="font-size: 12px; color: #666; margin-bottom: 12px;">These are auto-calculated from attendance. Leave empty if not applicable.</p>
            
            <div class="form-row">
                <div class="form-group">
                    <label for="late_deduction">Late Deduction</label>
                    <input type="number" step="0.01" name="late_deduction" id="late_deduction" value="{{ old('late_deduction', 0) }}" min="0" onchange="calculateDeductions()">
                </div>
                <div class="form-group">
                    <label for="absent_deduction">Absent Deduction</label>
                    <input type="number" step="0.01" name="absent_deduction" id="absent_deduction" value="{{ old('absent_deduction', 0) }}" min="0" onchange="calculateDeductions()">
                </div>
                <div class="form-group">
                    <label for="tax_deduction">Automatic Tax</label>
                    <input type="number" step="0.01" name="tax_deduction" id="tax_deduction" value="{{ old('tax_deduction', 0) }}" min="0" onchange="calculateDeductions()">
                </div>
            </div>
        </fieldset>
        ...
        <fieldset class="form-section">
            <legend>Government Deductions</legend>
            <p style="font-size: 12px; color: #666; margin-bottom: 12px;">All government deductions are manually entered. Admin must verify amounts.</p>
            
            <div class="form-row">
                <div class="form-group">
                    <label for="sss_manual">SSS (Social Security System)</label>
                    <input type="number" step="0.01" name="sss_manual" id="sss_manual" value="{{ old('sss_manual', 0) }}" min="0" onchange="calculateDeductions()" placeholder="0.00">
                </div>
                <div class="form-group">
                    <label for="philhealth_manual">PhilHealth</label>
                    <input type="number" step="0.01" name="philhealth_manual" id="philhealth_manual" value="{{ old('philhealth_manual', 0) }}" min="0" onchange="calculateDeductions()" placeholder="0.00">
                </div>
                <div class="form-group">
                    <label for="pag_ibig_manual">Pag-IBIG</label>
                    <input type="number" step="0.01" name="pag_ibig_manual" id="pag_ibig_manual" value="{{ old('pag_ibig_manual', 0) }}" min="0" onchange="calculateDeductions()" placeholder="0.00">
                </div>
                <div class="form-group">
                    <label for="tax_manual">Income Tax (BIR)</label>
                    <input type="number" step="0.01" name="tax_manual" id="tax_manual" value="{{ old('tax_manual', 0) }}" min="0" onchange="calculateDeductions()" placeholder="0.00">
                </div>
            </div>
        </fieldset>
